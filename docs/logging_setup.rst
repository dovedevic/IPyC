...
*IPyC* logs errors and debug information via the :mod:`logging` python
...
IPyC as well as other libraries that use the ``logging`` module
...
example to write the logs to a file called ``communication.log`` instead of
outputting them to the console the following snippet can be used::

    import ipyc
    import logging

    logger = logging.getLogger('IPyC')
    logger.setLevel(logging.DEBUG)
    handler = logging.FileHandler(filename='communication.log', encoding='utf-8', mode='w')
